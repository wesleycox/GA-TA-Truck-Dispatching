...
public class MainN {
	...
	public static void main(String[] args) {
		if (args.length < 4) {
			throw new IllegalArgumentException(String.format("\nusage: ... MainN filename numSamples runtime solIndex...\n" +
	...
	public void run(String file, int[] solIndexes, int numSamples, double runtime) {
		MineParametersN params = new MineParametersN(file);
		TimeDistribution tgen = new UniformTimes();
		MineSimulatorNarrowMCNWTL sim = new MineSimulatorNarrowMCNWTL(params,tgen);
		...
		for (int solIndex : solIndexes) {
			...
			switch (solIndex) {
				case 0: {
					...
					int resampleRate = 1;
					int resampleSize = 1;
					double smFactor = params.getNumShovels() + params.getNumCrushers();
					int fixedShovelLength = 0;
					boolean allowSurvivors = true;
					int mutN = 1;
					sol = new GACycleSolutionN(params,tgen,runtime)
						...
						.setStringMutationFactor(smFactor)
						.setShovelCycleLength(fixedShovelLength)
				...
				case 4: {
					...
					sol = new HeuristicSolutionN(params,hKinds[solIndex - 1],tgen,20,runtime);
					break;
				}
				case 5: {
					sol = new DISPATCHSolutionN(params,runtime,false);
					break;
				}
				default: {
