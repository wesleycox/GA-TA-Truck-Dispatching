...
/**
 * Two-stage greedy heuristic controller for complex road networks.
 * Routing is performed by greedily minimising a heuristic value to a shovel,
 * then given that decision,
 * minimising the same heuristic for the return route.
 ...
 */
public class HeuristicControllerNLC implements RouteController {
	...
	private static class GreedySimulator extends SimFitnessFunctionMCNWTL {
		...
		private double prevWaitingTime;	//the waiting time for the truck under consideration up the shovel.
		private double prevServiceEnd;	//the departure time from a shovel for the truck under consideration.
		...
		protected int nextRoute(int tid) {
			if (tid != currentDispatch) {
				...
				dispatches++;
				prevServiceEnd = currTime;
				prevWaitingTime = getTotalWaitingTime(currentDispatch);
				return returnRoute;
			}
		...
		/**
		 * Get the heuristic value for the considered truck given a schedule,
		 * averaged over multiple forward simulations.
		 * Possible heuristics are MTWT, MTST, and MSWT.
		 *
		 ...
		 * @param	kind			the heuristic type.
		 * @return	the average heuristic value.
		 */
		public double getValue(int currentDispatch, int[] assignedRoute, int returnRoute, double endtime, int numSamples, 
			...
			int expectedDispatches = 2 + (returnRoute < 0 ? 0 : 1);
			double total = 0;
					...
					switch (kind) {
						case MTWT: {
							if (returnRoute < 0) {
								...
								total += getTotalWaitingTime(currentDispatch) - prevWaitingTime;
							}
							break;
						}
						case MTST: {
							if (returnRoute < 0) {
								...
								total += getLastServiceStart(currentDispatch) - prevServiceEnd;
							}
							break;
						}
						case MSWT: {
							if (returnRoute < 0) {
								total += getServiceAvailableTime(currentDispatch) - simTime;
							}
							else {
								total += getServiceAvailableTime(currentDispatch) - prevServiceEnd;
							}
							break;
						}
						default: {
							throw new IllegalArgumentException(String.format("%s not supported",kind));
						}
					}
				}
				else {
	...
	private HeuristicKind kind;	//the heuristic type.
	 ...
	 * @param	kind				the heuristic type.
	 */
	public HeuristicControllerNLC(int numTrucks, int numCrushers, int numShovels, int numRoads, double[] emptyTimesMean, 
		double[] emptyTimesSD, double[] fillTimesMean, double[] fillTimesSD, double[] roadTravelTimesMean, double[] roadTravelTimesSD,
		double fullSlowdown, boolean[] isOneWay, int numRoutes, int[][] routeRoads, int[][] routeDirections, int[] routeLengths,
		int[] routeCrushers, int[] routeShovels, TimeDistribution tgen, double endtime, int numSamples, HeuristicKind kind) {
	 ...
	 * 
	 * The route to a shovel is chosen that minimises the set heuristic value,
	 * then given that route,
	 * schedule the return route to a crusher that will minimise the heuristic.
	...
	public int nextRoute(int tid) {
		...
		double[] values = new double[numAvailable];
		for (int i=0; i<numAvailable; i++) {
			dispatch[tid] = availableRoutes.get(i);
			values[i] = ff.getValue(tid,dispatch,-1,endtime + 200,numSamples,kind);
		}
		int first = availableRoutes.get(minIndex(values));
		availableRoutes = routesFromShovels[routeShovels[first]];
		numAvailable = availableRoutes.size();
		values = new double[numAvailable];
		dispatch[tid] = first;
		for (int i=0; i<numAvailable; i++) {
			int returnRoute = availableRoutes.get(i);
			values[i] = ff.getValue(tid,dispatch,returnRoute,endtime + 200,numSamples,kind);
		}
		int second = availableRoutes.get(minIndex(values));
		scheduledRoute[tid] = second;
		return first;
	}
